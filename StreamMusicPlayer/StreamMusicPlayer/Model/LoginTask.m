...
- (NSString *)getUrl {
    return [NSString stringWithFormat: @"%@/user/login", [[GlobalVars sharedInstance] getServerUrl]];
}
...
- (void)sendRequest:(NSString *)email Password:(NSString *)password FacebookId:(NSString *)facebookId Response:(void(^)(int, NSDictionary *))response {
    ...
    NSDictionary *params = @{@"email" : email, @"password" : password, @"facebook_id" : facebookId};
